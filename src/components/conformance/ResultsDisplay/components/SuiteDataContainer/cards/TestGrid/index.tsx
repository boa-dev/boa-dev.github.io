...
import { SpecEdition, TestOutcome, TestResult, SuiteResult } from "@site/src/components/conformance/types";
        ...
        <div className={styles.testGridCard}>
            <div className="card">
                ...
                <div className={cardBodyClass}>
                    <Grid tests={props.suite.tests} esFlag={props.esFlag} selectTest={props.selectTest} />
                </div>
            </div>
        </div>
    )
    ...
    return (
        <>
            ...
            ? props.tests.filter(test=>test.edition <= SpecEdition[props.esFlag]).map((test)=>{
                return <GridItem key={test.strict ? test.name + "-strict" : test.name} test={test} selectTest={props.selectTest} />
            })
