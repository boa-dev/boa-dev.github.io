...
Boa is now `boa_engine` on crates.io. This should bring it in line with out other packages, and allows us to resolve the casing issue we had with Boa previously. It should just be a case of changing your dependencies in cargo.toml to the new destination. The old `Boa` crate will no longer recieve any updates.
